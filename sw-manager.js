...
const VERSION      = '1';
const CACHE_STATIC = `rosterkit-manager-v${VERSION}`;
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_STATIC)
      ...
      .catch(err => console.warn('[SW Manager] Precache failed:', err))
  );
  self.skipWaiting();
});
  ...
  event.respondWith(
    caches.match(req).then(cached => {
      ...
      return fetch(req).then(response => {
        ...
        if (req.mode === 'navigate') return caches.match('./manager.html');
